...
if ! $IS_LINUX; then
  record "home-dir-perms" "PASS" "macOS: $homedir_perms (system role is Linux-only; not enforced)"
...
EOF
  ...
  if [[ "$EUID" -ne 0 ]]; then
    ...
    _bad_alg=""
    grep -qE '^Ciphers aes256-gcm@openssh\.com,chacha20-poly1305@openssh\.com$' /etc/ssh/sshd_config.d/00-hardening.conf 2>/dev/null || _bad_alg="${_bad_alg:-Ciphers}"
