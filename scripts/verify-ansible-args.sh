...
unset PAGER ANSIBLE_PAGER SYSTEMD_PAGER  # pager injection — ansible-vault view decrypts vault and pipes plaintext to the pager binary via subprocess.Popen(pager, shell=True, stdin=subprocess.PIPE) (cli/__init__.py line 527); Ansible reads ANSIBLE_PAGER first then PAGER as fallback (base.yml DEFAULT_PAGER); PAGER=/tmp/evil.sh exfiltrates all vault plaintext (SSH private keys, authorized_keys, registry tokens) to an attacker-controlled binary; SYSTEMD_PAGER is the systemd equivalent consumed by systemctl/journalctl pager invocations that may occur during provisioning tasks; clearing all three is defence-in-depth for any vault-view Makefile target routed through VERIFY_AND_RUN
unset PAGER MANPAGER SYSTEMD_PAGER GIT_PAGER  # pager injection — ansible-vault view/decrypt pipes vault plaintext (SSH private keys, credentials) through the binary named in PAGER > MANPAGER > SYSTEMD_PAGER > GIT_PAGER; PAGER=/tmp/evil.sh receives vault contents on stdin before the legitimate pager runs; site.yml:168 and SECURITY.md:691 direct users to run ansible-vault view directly, bypassing Makefile unexport and this script's sanitization for the editor/WGETRC vars already cleared above; unsetting all four prevents pager-chain exfiltration regardless of invocation path
