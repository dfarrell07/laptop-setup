...
unset XDG_CONFIG_HOME  # prevent registry credential hijacking — podman reads $XDG_CONFIG_HOME/containers/auth.json when XDG_CONFIG_HOME is set, overriding $HOME/.config; attacker sets XDG_CONFIG_HOME=/tmp/evil and places a crafted auth.json there, routing all podman pull/login calls to an attacker-controlled registry or injecting stolen tokens; unsetting forces podman to use $HOME/.config/containers/auth.json (managed by the vault and containers role)
