...
export XDG_CONFIG_HOME="${HOME}/.config"  # pin podman config dir — podman reads storage.conf, registries.conf, policy.json, and auth.json from $XDG_CONFIG_HOME/containers/; attacker sets XDG_CONFIG_HOME=/tmp/evil and pre-plants /tmp/evil/containers/storage.conf redirecting graphRoot to an attacker-controlled path, corrupting container provisioning (distrobox assemble create silently writes to /tmp/evil/graph); auth.json hijack also possible (routes podman pull/login to attacker registry); explicitly pinning to ${HOME}/.config (validated above against /etc/passwd) prevents both attacks without relying on fallback behaviour
...
unset CURL_HOME CURLOPT_CAPATH  # curl config injection: CURL_HOME=/tmp/evil causes curl to read /tmp/evil/.curlrc before any command-line flags, allowing 'proxy = http://attacker.com' or 'insecure' to route all ansible.builtin.shell curl invocations (repos_dnf/tasks/main.yml lines 256, 330, 394 — Red Hat internal GPG key, RPM Fusion free/nonfree GPG keys) through an attacker-controlled proxy before the gpg --with-colons fingerprint check runs; a pass-through proxy achieves silent credential observation; 'insecure' in .curlrc also strips TLS; CURLOPT_CAPATH overrides the CA certificate directory used by curl for TLS verification
...
unset DBUS_SYSTEM_BUS_ADDRESS  # D-Bus system bus socket poisoning — libdbus reads DBUS_SYSTEM_BUS_ADDRESS at the C level (before Python) and uses it in preference to the compiled-in /run/dbus/system_bus_socket default; FirewallClient.__init__() (firewall/client.py) calls dbus.SystemBus() with no explicit address argument, so an attacker-set DBUS_SYSTEM_BUS_ADDRESS=unix:path=/tmp/evil.sock routes all 16 ansible.posix.firewalld calls in roles/system/tasks/firewall.yml (drop zone, SSH port 722, ICMP rules, FedoraWorkstation zone removal, libvirt zone) and both calls in roles/system/tasks/nm.yml (tailscale0 trusted, wt0 trusted) to a fake daemon that returns success; nftables state is never changed; firewall hardening is silently bypassed while provisioning reports success; unsetting forces libdbus to connect to /run/dbus/system_bus_socket (canonical, non-spoofable)
unset DBUS_SYSTEM_BUS_ADDRESS  # D-Bus system bus socket poisoning — libdbus (dbus_bus_get(DBUS_BUS_SYSTEM)) and GDBus (used by nmcli/libnm) both honour this env var; ansible.posix.firewalld calls dbus.SystemBus() via python-firewall which resolves through the same path; an attacker-set DBUS_SYSTEM_BUS_ADDRESS=unix:path=/tmp/evil.sock routes firewall.yml firewalld tasks and nm.yml nmcli tasks to a cooperating fake daemon that returns exit 0 while actual firewalld zones and NetworkManager DNS configuration remain in their pre-provisioning state, silently bypassing firewall and NM hardening; the canonical system bus socket (/var/run/dbus/system_bus_socket) is non-configurable at runtime — no legitimate caller sets this variable before invoking make
...
unset CURL_HOME WGETRC  # curl/wget config injection — CURL_HOME=/tmp/evil causes bare curl calls (repos_dnf GPG key fetches) to read /tmp/evil/.curlrc; a .curlrc with 'output = /attacker-path' redirects where the GPG key file is written before the fingerprint check, potentially pointing at a pre-staged key; WGETRC similarly redirects wget output; neither shell task has an environment: block so both vars are fully inherited; unsetting forces curl/wget to use compiled-in defaults
