...
unset PIP_BREAK_SYSTEM_PACKAGES PIP_INDEX_URL PIP_EXTRA_INDEX_URL PIP_CERT PIP_CONFIG_FILE PIP_REQUIRE_VIRTUALENV PIP_TRUSTED_HOST  # pip env-var sanitization — PIP_BREAK_SYSTEM_PACKAGES=1 bypasses PEP 668 externally-managed protection; if any task invokes system pip under become:true an attacker-preset value overwrites system Python packages enabling root-accessible code injection; PIP_INDEX_URL/PIP_EXTRA_INDEX_URL redirect package fetches to attacker-controlled index (dependency confusion); PIP_CERT/PIP_CONFIG_FILE inject attacker-controlled TLS trust or pip config; PIP_REQUIRE_VIRTUALENV=1 could block legitimate pip use in provisioning tasks; PIP_TRUSTED_HOST disables TLS certificate verification for a named host independently of PIP_INDEX_URL — attacker pre-sets PIP_TRUSTED_HOST=pypi.org so that after PIP_INDEX_URL is unset pip falls back to https://pypi.org/simple/ with cert verification suppressed, enabling rogue-AP/DNS-poisoning MITM of pipx installs (yamllint, ansible-lint, transitive deps) without needing a redirect
...
unset ANSIBLE_INVENTORY_ANY_UNPARSED_IS_FAILED  # DoS via inventory parse failure: attacker pre-sets this to True; default is False; with True, Ansible hard-aborts if any inventory source produces a parse warning — provides a lever to break provisioning against any inventory change or malformed file without code execution
# --- Ansible output/behavior ---
...
unset ANSIBLE_COW_PATH ANSIBLE_COW_SELECTION ANSIBLE_COW_ACCEPTLIST  # cowsay binary/template injection
export ANSIBLE_NOCOWS=1  # ANSIBLE_NOCOWS causes set_cowsay_info() to return before reading ANSIBLE_COW_PATH; load-bearing mitigation; unset above is defense-in-depth for child processes
...
unset ANSIBLE_FORCE_COLOR  # ANSI escape injection — ANSIBLE_FORCE_COLOR=1 forces ANSI SGR codes (\x1b[...m) into ansible-playbook stdout even when piped to tee/CI artifact storage; the default callback (ANSIBLE_STDOUT_CALLBACK=default, pinned above) emits CSI sequences for task status (ok/changed/failed coloring); in ANSI-aware log viewers, attacker-composed sequences embedded in task output (debug: msg=, fail: msg=) can overwrite preceding terminal lines (\x1b[A + \x1b[2K) to hide hardening-failure messages from operators watching the stored log; unset lets the callback detect whether stdout is a tty and suppress codes when piped, restoring correct log-viewer rendering
...
export XDG_CONFIG_HOME="${HOME}/.config"  # pin podman config dir — podman reads storage.conf, registries.conf, policy.json, and auth.json from $XDG_CONFIG_HOME/containers/; attacker sets XDG_CONFIG_HOME=/tmp/evil and pre-plants /tmp/evil/containers/storage.conf redirecting graphRoot to an attacker-controlled path, corrupting container provisioning (distrobox assemble create silently writes to /tmp/evil/graph); auth.json hijack also possible (routes podman pull/login to attacker registry); explicitly pinning to ${HOME}/.config (validated above against /etc/passwd) prevents both attacks without relying on fallback behaviour; placed after unset block so the pin takes effect last (clear attacker value, then set safe value)
...
unset VAULT_TOKEN VAULT_ADDR VAULT_NAMESPACE VAULT_SKIP_VERIFY VAULT_CACERT  # prevent HashiCorp Vault server redirect and TLS bypass — current blast radius is zero (no community.hashi_vault usage); gap opens silently on the first vault_kv_get lookup or hashi_vault role import, at which point an attacker-preset VAULT_ADDR=https://evil.com and VAULT_TOKEN=attacker-token redirect all HCV API calls to an attacker-controlled Vault server with no existing guard; VAULT_NAMESPACE poisons the namespace context for multi-tenant Vault deployments; VAULT_SKIP_VERIFY=1 disables TLS certificate verification for every HCV API connection enabling MITM of secret reads; VAULT_CACERT=/tmp/evil.pem substitutes an attacker-controlled CA for TLS trust achieving the same MITM result with a valid-looking certificate
unset CONSUL_HTTP_TOKEN CONSUL_HTTP_ADDR CONSUL_HTTP_SCHEME CONSUL_HTTP_SSL_VERIFY CONSUL_CACERT  # prevent Consul API redirect and TLS bypass — current blast radius is zero (no consul_io.consul or community.general.consul usage in the codebase); gap opens silently on the first consul_kv lookup or consul role import: CONSUL_HTTP_ADDR=https://evil.com redirects all Consul API calls to an attacker-controlled server; CONSUL_HTTP_TOKEN sends the token to that server; CONSUL_HTTP_SCHEME=http downgrades TLS to plaintext; CONSUL_HTTP_SSL_VERIFY=false disables TLS certificate verification enabling MITM of all Consul reads/writes; CONSUL_CACERT=/tmp/evil.pem injects a malicious CA achieving the same MITM result with a valid-looking certificate
unset HELM_DATA_HOME HELM_CONFIG_HOME HELM_CACHE_HOME HELM_NAMESPACE HELM_DEBUG HELM_REPOSITORY_CONFIG HELM_REPOSITORY_CACHE HELM_REGISTRY_CONFIG  # Helm env injection — current blast radius zero (only `helm version` is invoked, which reads compiled-in data and ignores these vars); gap opens silently without any code change if a future task adds helm install/repo add/upgrade: HELM_DATA_HOME redirects plugin directory enabling plugin hijack; HELM_CONFIG_HOME redirects config including pluginsDirectory and registry credentials; HELM_NAMESPACE poisons default namespace for cluster operations; HELM_REPOSITORY_CONFIG/HELM_REPOSITORY_CACHE redirect repo index and cache to attacker-controlled paths; HELM_REGISTRY_CONFIG redirects OCI registry credentials; HELM_DEBUG leaks internal state to stderr captured by Ansible registered vars
...
unset CC CXX CFLAGS CXXFLAGS CGO_CFLAGS CGO_CXXFLAGS LDFLAGS CGO_LDFLAGS CGO_ENABLED  # CGO compiler injection — CC=/tmp/evil-gcc or CGO_CFLAGS='-fplugin=/tmp/evil.so' propagates through ansible-playbook into every child go install subprocess; for CGO-enabled packages (subctl and golangci-lint in packages_go_install_work) the attacker-controlled compiler or compiler flags run during the build, allowing arbitrary code injection into the produced binary; CGO_ENABLED=0 in caller env could also disable CGO silently changing binary behaviour for packages that require it; unsetting all CGO/compiler vars forces go install to use the system compiler at its default path with no injected flags
# --- TLS CA injection ---
...
unset SYSTEMD_LOG_LEVEL  # systemd debug verbosity injection — AnsibleModule.run_command() starts with os.environ.copy() (confirmed: ansible/module_utils/basic.py line 118), so SYSTEMD_LOG_LEVEL propagates from the caller through ansible-playbook → AnsiballZ module subprocess → every systemctl/firewall-cmd child process; SYSTEMD_LOG_LEVEL=debug causes libsystemd (linked into systemctl, firewall-cmd, authselect, sshd -t) to emit verbose debug messages to stderr, which Ansible captures in the 'err' field of registered vars; with ANSIBLE_VERBOSITY=0 (pinned above) these do not appear in normal terminal output, but when failed_when conditions trigger, the full registered result including stderr is printed, exposing internal service state (NM zone assignments, firewalld rule details, sshd config validation context) in error output; SYSTEMD_LOG_LEVEL=debug also enlarges the journal entries written by every daemon restart handler (Restart sshd, Apply firewall rules) — on hosts where the journal is forwarded to an external aggregator the debug burst increases the volume of systemd-internal state exported off-host during the privileged provisioning window
unset SYSTEMD_LOG_TARGET  # systemd log-target injection — SYSTEMD_LOG_TARGET is also propagated via os.environ.copy() to every systemctl/firewall-cmd subprocess; SYSTEMD_LOG_TARGET=null discards ALL log output from those processes at all severity levels (libsystemd sd_journal_print() calls return silently), stripping error messages from systemctl's stderr even when the command exits non-zero; when ansible.builtin.command tasks use 'failed_when: false' (authselect, crypto-policies, audit rules — all present in system role) a failed command produces empty err='', depriving the operator of the error text needed to diagnose incomplete hardening; SYSTEMD_LOG_TARGET=kmsg redirects systemctl debug output to the kernel ring buffer (dmesg), which is readable by any process with CAP_SYSLOG (default: unrestricted on Fedora for the invoking user), persisting provisioning-state details across session boundaries after the sudo window closes
unset SYSTEMD_UNIT_PATH  # systemd unit search path injection — SYSTEMD_UNIT_PATH prepends attacker-controlled directories to systemd's unit file search path; when ansible.builtin.systemd starts or masks a unit by short name (e.g. 'firewalld', 'sshd.socket'), systemd resolves the name by walking SYSTEMD_UNIT_PATH before /usr/lib/systemd/system/; an attacker-planted firewalld.service at the injected path that exits 0 immediately without opening any D-Bus interface causes 'state: started' to succeed while firewalld is not actually running, silently bypassing all 16 subsequent ansible.posix.firewalld calls (which connect via D-Bus to the real firewalld socket, now absent); unset restores the compiled-in search path
...
unset PAGER ANSIBLE_PAGER MANPAGER SYSTEMD_PAGER SYSTEMD_PAGER_FLAGS GIT_PAGER  # pager injection — ansible-vault view decrypts vault and pipes plaintext to the pager binary via subprocess.Popen(pager, shell=True, stdin=subprocess.PIPE) (cli/__init__.py line 527); Ansible reads ANSIBLE_PAGER first then PAGER as fallback (base.yml DEFAULT_PAGER); PAGER=/tmp/evil.sh exfiltrates all vault plaintext (SSH private keys, authorized_keys, registry tokens) to an attacker-controlled binary; MANPAGER and GIT_PAGER are fallback pager env vars consumed by man/git respectively (git-diff, git-log pipe through GIT_PAGER); SYSTEMD_PAGER is the systemd equivalent consumed by systemctl/journalctl; SYSTEMD_PAGER_FLAGS passes additional flags to whatever pager systemctl/journalctl invokes — with SYSTEMD_PAGER cleared the fallback pager is the compiled-in default (typically less); SYSTEMD_PAGER_FLAGS='+G' or '-r' can alter how interactive pager output is rendered in make targets (make smoke-test, csb-audit) that invoke systemctl status and pipe through less; clearing prevents flag injection into the pager invocation chain; clearing all six prevents pager-chain exfiltration regardless of invocation path; defence-in-depth for vault-view/vault-diff Makefile targets routed through VERIFY_AND_RUN
export PATH=/usr/local/bin:/usr/bin:/bin  # pin PATH — prevents PATH=/attacker:$PATH hijacking args[0] resolution
export TZ=UTC  # pin timezone — ansible_date_time gathered facts (.tz, .date, .time, .hour) are derived from the process TZ; a pre-set TZ=America/Los_Angeles shifts all gathered timestamps, degrading forensic correlation between Ansible output and auditd/journal records; current blast radius is zero (no ansible_date_time references in roles/**/*.yml or roles/**/*.j2) but any future task using ansible_date_time for file naming, conditional logic, or timestamped output would silently receive wrong values; UTC is the conventional timezone for provisioning and audit systems
...
unset PAM_SERVICE PAM_RHOST PAM_TTY PAM_TYPE PAM_USER  # PAM environment variable defence-in-depth — these vars are evaluated by PAM modules at pam_authenticate()/pam_acct_mgmt() time, not during pam.d config writes or authselect commands; current provisioning tasks (authselect enable-feature, faillock.conf writes, lineinfile PAM stack insertion) do not call pam_authenticate so blast radius is zero in the current playbook; PAM_RHOST is evaluated by pam_access.so and pam_rhosts.so at auth time — if a future provisioning task calls su or runuser (which invoke PAM), a pre-set PAM_RHOST=attacker.host could influence access.conf policy evaluation; PAM_TTY affects pam_securetty and pam_issue at login time; PAM_TYPE is set internally by PAM to indicate the service-module type (auth/account/session/password) — an inherited value could confuse modules that inspect it; clearing all five is defence-in-depth for future roles that may invoke PAM-aware commands
