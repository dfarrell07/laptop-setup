...
export ANSIBLE_GATHER_SUBSET=all  # pin fact collection to full subset — ANSIBLE_GATHER_SUBSET='!all,!any' produces an empty ansible_facts dict; repos_dnf when: ansible_facts['pkg_mgr'] raises AnsibleUndefinedVariable aborting Play 1; site.yml Set OS family facts uses ansible_facts['os_family'] with no default filter aborting Play 2; ssh role never runs, no authorized_keys is written, sshd restarts to port 722 — lockout
...
unset ANSIBLE_GATHER_SUBSET  # gather_subset suppression: ANSIBLE_GATHER_SUBSET=all,!network removes ansible_fqdn; csb_detect.yml '| default("")' silently yields '' so _is_csb_hostname=False; on Fedora CSB hybrid (no fapolicyd) both detection paths fail, csb_detected=False, and hybrid-tier hardening is bypassed entirely
