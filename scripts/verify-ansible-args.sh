...
unset CONTAINERS_CONF CONTAINER_HOST DOCKER_HOST PODMAN_HOST  # prevent socket/runtime hijacking — attacker-set CONTAINERS_CONF=/tmp/evil/containers.conf can point [engine] runtime at a malicious OCI runtime binary that executes with every podman call (distrobox assemble create, podman login, podman pull) while exiting 0; attacker-set CONTAINER_HOST/DOCKER_HOST/PODMAN_HOST redirects podman API calls (including vault credential writes via podman login) to an attacker-controlled socket
