...
- **System role task split**: `roles/system/tasks/main.yml` (orchestrator, 730 lines) delegates to 14 sub-files — `mounts.yml` (filesystem/tmpfs), `firewall.yml` (firewalld), `kernel_lockdown.yml` (grubby kernel cmdline), `auditd.yml` (audit rules), `service_masking.yml` (mask/unmask), `logind.yml` (lid/suspend), `services.yml` (enable/disable), `sshd.yml` (SSHD hardening), `nm.yml` (NetworkManager), `session_timeout.yml` (TMOUT/shell timeout), `pam.yml` (PAM/faillock), `account_hardening.yml` (accounts/passwords), `dconf.yml` (GNOME dconf hardening), and `aide.yml` (AIDE IDS); edit the sub-file, not main.yml, when touching those subsystems (15 files total including main.yml)
