...
- **System role task split**: `roles/system/tasks/main.yml` (orchestrator) delegates to 10 sub-files — `mounts.yml` (filesystem/tmpfs hardening), `firewall.yml` (firewalld zones/ports), `kernel_lockdown.yml` (grubby kernel cmdline lockdown), `auditd.yml` (auditd rules/service), `service_masking.yml` (service masking), `services.yml` (service enable/disable), `sshd.yml` (SSHD hardening + SELinux port + crypto policy), `pam.yml` (PAM stack authselect/faillock), `aide.yml` (AIDE IDS), and `account_hardening.yml` (account lockout/password policy); edit the sub-file, not main.yml, when touching those subsystems (11 files total)
