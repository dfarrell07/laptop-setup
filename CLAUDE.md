...
2. Create `config.yml` with at minimum `desktop_environment: sway` (or `i3`/`gnome`) and
   ...
   the notes repo clone fails (private repo). If this is not the `dfarrell07` account, also
   add `notes_enabled: false` to prevent attempting to clone a private notes repo you don't have.
